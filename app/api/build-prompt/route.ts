...
import { generateBuildPromptFallback } from "@/lib/fallbackGenerator";
...
export async function POST(req: Request) {
  ...
  if (!result.ok) {
    const fallback = generateBuildPromptFallback(lead, platform || "lovable");
    return NextResponse.json({
      source: "local",
      prompt: fallback.prompt,
      pitchPoints: fallback.pitchPoints,
    });
  }
  return NextResponse.json({
    source: "claude",
