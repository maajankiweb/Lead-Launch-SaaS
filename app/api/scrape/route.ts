...
export async function POST(req: NextRequest) {
  try {
    ...
    // Enforce plan limits (e.g. Free = 15 max)
    let requestedCount = Math.max(1, input.count || 10);
    let planCapped = false;

    if (requestedCount > planConfig.limits.maxLeadsPerScrape) {
      requestedCount = planConfig.limits.maxLeadsPerScrape;
      planCapped = true;
    }
    ...
    if (!APIFY_TOKEN) {
      ...
      const sliced = leads.slice(0, Math.min(requestedCount, leads.length));
      return NextResponse.json({
        source: "seed",
        leads: sliced,
        plan: planConfig.id,
        planCapped,
        maxAllowed: planConfig.limits.maxLeadsPerScrape,
      });
    }
    ...
    const leads: Lead[] = items.slice(0, requestedCount).map((it, i) => ({
