...
import { generateAuditFallback } from "@/lib/fallbackGenerator";
...
export async function POST(req: Request) {
  ...
  if (!result.ok) {
    // If Claude Code CLI is unavailable or not signed in, seamlessly provide smart local audit
    const fallbackAudits = generateAuditFallback(leads);
    return NextResponse.json({ source: "local", audits: fallbackAudits });
  }
