...
import { generateRankFallback } from "@/lib/fallbackGenerator";
...
export async function POST(req: Request) {
  ...
  if (!result.ok) {
    const fallbackRanked = generateRankFallback(leads, audits);
    return NextResponse.json({ source: "local", ranked: fallbackRanked });
  }
