...
import { Crown, IndianRupee, MessageCircle, Phone, Mail, Sparkles, FileText, Download } from "lucide-react";
...
export function Phase3Rank({
  ...
  function exportRankedToCsv() {
    ...
    const rows = ranked.map((l, i) => [
      ...
      l.reviewsCount ?? "",
      l.score,
      `"${(l.website || "None").replace(/"/g, '""')}"`,
      l.audit.pageSpeedScore,
      `"${l.phone || ""}"`,
      l.audit.estLostRevenuePerMonth,
      `"${(l.scoreReasoning || "").replace(/"/g, '""')}"`
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(","), ...rows.map((e) => e.join(","))].join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `ranked-prospects-${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.success(`${ranked.length} ranked leads exported to CSV!`);
  }
    ...
    >
      <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {ranked.length > 0 && (
            <Button variant="outline" size="sm" onClick={exportRankedToCsv} className="h-10 text-xs">
        ...
        <>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>All ranked prospects ({ranked.length})</CardTitle>
              <Button
                size="sm"
                variant="outline"
                onClick={() => {
                  const headers = ["Rank", "Name", "Category", "Score", "ScoreReasoning", "LostRevenuePerMonth", "PageSpeedScore", "HasWebsite", "Phone", "WhatsApp", "Email"];
                  const rows = ranked.map((l, i) => [
                    i + 1,
                    `"${(l.name || "").replace(/"/g, '""')}"`,
                    `"${(l.category || "").replace(/"/g, '""')}"`,
                  ...
                  link.click();
                  document.body.removeChild(link);
                  toast.success("Exported ranked prospects to CSV!");
                }}
                className="text-xs h-8"
              >
                <Download className="h-3.5 w-3.5 mr-1.5" /> Export Ranked CSV
              </Button>
            </CardHeader>
            <CardContent>
