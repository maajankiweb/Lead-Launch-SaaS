          ...
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            ...
            >
              <motion.div
                ...
                className={cn(
                  "h-9 w-9 sm:h-11 sm:w-11 rounded-full flex items-center justify-center border",
                  isCurrent
              ...
              >
                {isDone && !isCurrent ? <Check className="h-3.5 w-3.5 sm:h-4 sm:w-4" strokeWidth={1.75} /> : <Icon className="h-3.5 w-3.5 sm:h-4 sm:w-4" strokeWidth={1.5} />}
              </motion.div>
              <span className={cn("text-[9px] sm:text-[11px] tracking-wide uppercase text-center", isCurrent ? "text-foreground font-medium" : "text-muted-foreground")}>
                <span className="hidden xs:inline">{String(step.id).padStart(2, "0")} · </span>{step.label}
              </span>
            </button>
            {i < STEPS.length - 1 && (
